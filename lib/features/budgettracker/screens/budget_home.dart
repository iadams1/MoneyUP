...
class _BudgetGoalPageState extends State<BudgetGoalPage> {
  ...
  Future<void> _loadBudgetGoalPage() async {
    setState(() {
      _isLoading = true;
    });
    ...
    try {
      ...
      debugPrint('Error loading budget goal page: $e');
      if (!mounted) return;
  ...
  Future<void> loadSpendingData() async {
    ...
    final response = await budgetService.getSpendingRows(
      start: range.start,
      end: range.end,
    );
